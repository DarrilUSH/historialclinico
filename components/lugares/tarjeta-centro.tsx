 ...
 * **"Cómo llegar"** y **"Pedir viaje"**, con las MISMAS funciones de
 ...
 * "Pedir viaje" puede existir, porque Uber/DiDi/Cabify SOLO aceptan
 * coordenadas. Cuando no están, "Cómo llegar" cae en la búsqueda por
 * dirección y "Pedir viaje" no se muestra: exactamente el mismo
 * comportamiento que un turno sin coordenadas, sin código nuevo.
 *
 * Es cliente y no servidor por una sola razón: "Pedir viaje" es un panel
 * expandible con estado (mismo patrón que `components/turnos/acciones-turno.tsx`).
 */

import * as React from "react"

import { CarIcon, ChevronDownIcon, ExternalLinkIcon, MapPinIcon } from "lucide-react"
...
import { linkComoLlegar, linksPedirViaje } from "@/lib/logistica/deep-links"
...
export function TarjetaCentro({ centro }: { centro: CentroEnTarjeta }) {
  const [expandidoViaje, setExpandidoViaje] = React.useState(false)
  ...
  const viajes = linksPedirViaje({
    latitude: centro.latitude,
    longitude: centro.longitude,
    nombreLugar: centro.name,
  })
  const hayViajes = viajes.uber !== null || viajes.didi !== null || viajes.cabify !== null
  ...
  return (
    <article className={cn(CLASE_TARJETA_BASE, "px-(--card-spacing)")}>
      ...
      <div className="flex flex-col gap-2 border-t border-border pt-2 chica:flex-row chica:flex-wrap chica:items-start">
        ...
        {hayViajes && (
          <div className="flex flex-col gap-2 chica:min-w-[40%] chica:flex-1">
            <Boton
              type="button"
              onClick={() => setExpandidoViaje((valor) => !valor)}
              variant="secondary"
              size="sm"
              aria-expanded={expandidoViaje}
            ...
            >
              ...
              <ChevronDownIcon
                className={cn(
                  "size-4 transition-transform chica:size-3.5",
                  expandidoViaje && "rotate-180",
                )}
                aria-hidden="true"
              />
            </Boton>
            ...
            {expandidoViaje && (
              <div className="space-y-2 border-l border-muted-foreground pl-4">
                {viajes.uber && <EnlaceViaje href={viajes.uber} nombre="Uber" />}
                {viajes.didi && <EnlaceViaje href={viajes.didi} nombre="DiDi" />}
                {viajes.cabify && <EnlaceViaje href={viajes.cabify} nombre="Cabify" />}
              </div>
            )}
          </div>
        )}
...
function EnlaceViaje({ href, nombre }: { href: string; nombre: string }) {
  return (
    <Boton
