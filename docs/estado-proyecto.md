...
- **Próximo:** 17.2 (barrido de la etiqueta + ingesta al pipeline de documentos + filtros por remitente aprendidos + flujo de revisión). Deuda S17: migración de contracción (`drop column doctors.specialty` + su índice muerto). PASO DEL USUARIO al despertar: en /lugares tocar "Actualizar" UNA vez para poblar el catálogo de producción (36.046 centros, ~10s). Backlog: cruce analizador↔catálogo REFES; lector XLSX del REFES; aceptación de invitaciones; temas para la consulta; cerrar/limitar registro público (decisión pendiente).
