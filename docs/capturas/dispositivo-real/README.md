...
| sprint17-gmail-inicio.png | `/inicio` en CHICA (411px reales) con la grilla de accesos ya en **3 filas justas de 3**: la card nueva "Tu Gmail" cierra la tercera fila, al lado de "Ficha médica" y "Ficha SOS". Es la única card de esa grilla que no habla del perfil activo -la casilla es de la CUENTA-, y por eso va última |
| sprint17-gmail-pantalla.png | `/perfil/gmail` DESCONECTADO en el dispositivo real: la explicación completa en castellano llano ("Solo los correos de la etiqueta `historialmedico`", "creamos esa etiqueta", "Nada más de tu casilla", "nada se guarda sin que vos lo revises", "podés desconectarlo cuando quieras") y el botón grande "Conectar mi Gmail". Entra ENTERA sin scrollear: `scrollHeight` 775px contra el viewport del A71 |
| sprint17-gmail-consent.png | Después del TAP real en "Conectar mi Gmail": el navegador salió a `accounts.google.com` con **"Ir a Historial Médico"** en pantalla — el nombre de la pantalla de consentimiento del proyecto `gen-lang-client-0873820464`, o sea la prueba de que el `redirect` del Route Handler llegó bien (client_id, scopes, state y `redirect_uri` correctos). **No se inició sesión ni se consintió nada**: ese toque es del usuario. La fila con la cuenta personal del dueño del equipo está TAPADA a propósito — el repositorio es público |
| sprint17-gmail-conectada.png | Estado CONECTADO en el dispositivo (conexión sembrada en la base LOCAL con `guardar_conexion_gmail` y un token de mentira, para poder fotografiar la pantalla sin el consentimiento real): casilla, "Conectada desde", "Etiqueta que leemos `historialmedico` ✓", "Última revisión" y el botón "Desconectar" |
| sprint17-gmail-vencida.png | Estado VENCIDO (`marcar_conexion_gmail_vencida`, que es lo que la app hace cuando Google contesta `invalid_grant` — el caso ESPERADO cada 7 días mientras la pantalla de consentimiento siga "En prueba"): "Se venció el permiso", la explicación de que no se perdió nada, y los dos botones "Volver a conectar" / "Desconectar" |
| sprint17-gmail-desconectar.png | Diálogo de confirmación de la desconexión, con la consecuencia en una línea y la casilla nombrada: "La app deja de leer los correos de … ahora mismo y le pedimos a Google que anule el permiso" |
| sprint17-gmail-velo.png | El velo de espera durante la desconexión real ("Desconectando tu Gmail / Le estamos avisando a Google que anule el permiso"). Esta captura es además la evidencia del bug del velo que se encontró y arregló en esta tarea (ver abajo) |
| sprint17-gmail-desconectada.png | Resultado REAL de tocar "Sí, desconectar" en el teléfono: éxito ("La app ya no puede leer ningún correo tuyo") **más** el aviso honesto de revocación pendiente -Google rechazó el token de mentira, que es exactamente el camino que la app tiene que saber contar- con el enlace a los permisos de la cuenta de Google, y debajo la pantalla ya en estado desconectado. Verificado contra la base: 0 conexiones y **0 secretos en el Vault** |
| tarea14-consejo-instalar-app.png | `/inicio` en el dispositivo real, cuenta nueva sin datos: el consejo "Instalá la app en tu teléfono" (prioridad 1) arriba de la grilla, con "Ahora no"/"No mostrar más" — la condición (`instalar_app`) es 100% client-conocible y se resolvió tras montar, sin parpadeo visible |
| tarea14-consejo-sos-desaparecio.png | La MISMA pantalla, dos pasos después: se pospuso el consejo 1 ("Ahora no"), se completó la ficha SOS de verdad (nombre de contacto), y el consejo de "Completá tu ficha de emergencia" **desapareció solo** -confirmado además navegando de nuevo, no volvió-. El siguiente consejo mostrado es "Conectá tu Gmail" (prioridad 4): el de notificaciones (3) se salteó porque `Notification.permission` ya era `"granted"` en ese origen real del dispositivo, condición leída en vivo del navegador |
| tarea14-ayuda-checklist.png | `/ayuda` ("¿Cómo empiezo?"): los seis pasos con su estado real -Pasos 2 y 3 en ✓ Hecho (ficha SOS completada, notificaciones ya con permiso), el resto Pendiente con su link/botón- |
| tarea14-ayuda-faq.png | La misma pantalla, con la primera pregunta del mini-FAQ ("¿Qué lee la aplicación de mi Gmail?") abierta por un tap real sobre el `<summary>` |

## Tarea #14 — Tutorial de bienvenida: consejos contextuales y "¿Cómo empiezo?"

**2026-08-18.** Samsung Galaxy A71 real (`R58N85AW49F`), `adb reverse tcp:3000 tcp:3000` + `adb reverse tcp:54321` (ya activos de una sesión anterior) + `adb forward tcp:9222 localabstract:chrome_devtools_remote`, CDP nativo por WebSocket de Node 24 (mismo método que el resto de este archivo). Ancho CSS real de celular, modo CHICA.

**La sesión se instaló por COOKIE, no tecleando la contraseña — con una cuenta NUEVA.** A diferencia de las verificaciones anteriores (que reutilizan a María del seed), esta tarea necesitaba una cuenta CON TODO pendiente. Se creó `tutorial.bienvenida@ejemplo.com.ar` contra la API de auth de Supabase LOCAL (`POST /auth/v1/signup`, sin tocar ningún campo de contraseña en el dispositivo — la clave la usó únicamente `curl` desde la terminal, nunca el teclado del teléfono) y la sesión resultante (`access_token`/`refresh_token`) se inyectó con `Network.setCookie`, en dos cookies `sb-127-auth-token.0`/`.1` -la sesión superó los 3180 bytes del umbral de chunking de `@supabase/ssr` (`node_modules/@supabase/ssr/dist/main/utils/chunker.js`), así que se replicó el mismo particionado que hace el SDK, no una sola cookie larga- más `perfil_activo` y `tamano`. Se encontró y corrigió en el camino una cookie `sb-127-auth-token` SIN chunkear que había quedado de una sesión anterior en la misma pestaña: `combineChunks` la lee PRIMERO que a los chunks nuevos, así que hay que borrarla explícitamente antes de escribir (`Network.deleteCookies`) o la inyección nueva queda invisible.

**Circuito completo verificado, con datos reales de la interacción (no simulados):**

1. Cuenta nueva → `/inicio` muestra el consejo 1 ("Instalá la app en tu teléfono"), el de mayor prioridad entre los seis, resuelto client-side tras montar (viewport de celular + no instalada).
2. "Ahora no" (tap real por coordenadas CSS reales del botón, `getBoundingClientRect` + `Input.dispatchMouseEvent`) → fila `pospuesto` en `consejos_estado` confirmada por SQL, y la pantalla cambia AL TOQUE al consejo 2 ("Completá tu ficha de emergencia") — la revalidación de la Server Action (`revalidatePath("/inicio")`) alcanza para que el siguiente consejo aparezca en la misma visita, no hace falta esperar a la próxima.
3. CTA "Completar ficha SOS" → aterriza en `/perfil/sos` (vía el Route Handler nuevo `/perfil/sos/enlace`) con el perfil PROPIO activo. Se completó el campo "Nombre" del contacto de emergencia (con el mismo truco de setter nativo + `dispatchEvent` que ya documenta este archivo) y se guardó de verdad.
4. Al volver a `/inicio`: el consejo de la ficha SOS **desapareció solo** -confirmado por SQL (sin fila en `consejos_estado` para `ficha_sos`: nunca se descartó, la condición dejó de ser cierta-, y por una segunda visita que tampoco lo trajo de vuelta.
5. El consejo siguiente fue "Conectá tu Gmail" (prioridad 4), NO "Activá las notificaciones" (3): `Notification.permission` ya era `"granted"` en ese origen del dispositivo real (de pruebas de push de sprints anteriores) — confirma que la condición se lee de verdad del navegador, no de un estado simulado.
6. "No mostrar más" en Gmail → fila `descartado` confirmada por SQL, cascada al consejo 5 ("Compartí con tu familia").
7. **Persistencia intra-sesión**: se navegó a `/turnos` y de vuelta a `/inicio` sin tocar ningún consejo — siguió mostrando "Compartí con tu familia", el pospuesto de la app NO reapareció.
8. **Reaparición en sesión NUEVA**: se borró la cookie `consejos_sesion` por CDP (`Network.deleteCookies`, simula el cierre completo del navegador -el mecanismo real, ver `docs/tutorial-bienvenida.md` §4-) y, al volver a `/inicio`, "Instalá la app en tu teléfono" reapareció y volvió a ganarle en prioridad al resto.
9. `/familia/enlace?perfil=<propio>` aterrizó en `/familia#invitar` con la sección visible; `/familia#crear-perfil-gestionado` (sin pasar por ningún `.../enlace`, es independiente del perfil activo) también.
10. `/ayuda`: los seis pasos con su estado real -incluido Gmail, que en `/inicio` ya estaba **descartado** pero acá sigue apareciendo como **Pendiente** con su link: confirma que `/ayuda` ignora el descarte, tal como se diseñó-. El acordeón de preguntas frecuentes abre con un tap real.

**Data de desarrollo:** ninguna. La cuenta (`tutorial.bienvenida@ejemplo.com.ar`) se creó y se borró dentro de esta verificación (`delete from auth.users` en cascada); la base local terminó con **0 filas nuevas en `consejos_estado`**, confirmado por SQL.

**Suites del cierre:** `npx tsc --noEmit` limpio · `npx eslint .` limpio (incluida la corrección de `react/no-unescaped-entities` en `/ayuda` y de `react-hooks/set-state-in-effect` en los dos Client Components nuevos) · `npm run test` **1240/1240** · `node scripts/verificar-contraste.mjs` **196/196** (sin pares nuevos) · RLS `scripts/test-rls.sql` **516/516 ×2** (BLOQUE 26: 20 casos) · `npm run build` limpio con el puerto 3000 verificado por `netstat` antes.

## Sprint 17 · tarea 17.1 — Conectá tu Gmail: OAuth, tokens y la etiqueta

**2026-08-18.** Samsung Galaxy A71 real (`R58N85AW49F`), `adb reverse tcp:3000 tcp:3000` + `adb reverse tcp:54321` + `adb forward tcp:9222 localabstract:chrome_devtools_remote`, CDP nativo por WebSocket de Node 24 (mismo método que el resto de este archivo). Ancho CSS real 411px, modo CHICA.

**La sesión se instaló por COOKIE, no tecleando la contraseña.** La sesión de María (cuenta del seed) se pidió a la API de auth de Supabase LOCAL y se inyectó con `Network.setCookie` (`sb-127-auth-token` + `perfil_activo` + `tamano`). Es más determinístico que tipear en el formulario y no arriesga la hoja del password manager de Chrome del dueño del equipo, que este archivo ya documenta como problemática más abajo.

**El límite declarado de la tarea:** el consentimiento final de Google exige la cuenta de Google del usuario, y **ese toque no se dio**. La captura `sprint17-gmail-consent.png` prueba lo que sí se puede probar sin él: que el tap en "Conectar mi Gmail" sale de la app y aterriza en la pantalla de Google **de esta aplicación** ("Ir a Historial Médico"). Nada se autorizó, ninguna cuenta se agregó.

**Un bug REAL encontrado acá y en ningún otro lado.** Tocar "Sí, desconectar" en el teléfono devolvió, la primera vez, `A "use server" file can only export async functions, found object` (500): `actions.ts` exportaba el objeto `ESTADO_GMAIL_INICIAL` además de la acción. **El `npm run build` compila limpio con ese error** —solo explota al invocar la acción—, así que ninguna suite lo habría detectado. Se movió el estado inicial a `lib/gmail/conexion.ts`.

**Y un segundo bug, este preexistente y de un componente COMPARTIDO.** Con el primero arreglado, la desconexión funcionó pero **el velo de espera quedó montado para siempre**, con toda la página `inert` detrás: la pantalla quedaba muerta. Diagnóstico medido en el dispositivo (sonda `data-*` temporal + lectura del DOM por CDP): el `isPending` de `useActionState` ya valía `false` y el velo seguía ahí. La causa es una ventana real en `components/base/velo-espera.tsx` (Sprint 14): el ajuste de estado que apaga `montado` corre EN EL RENDER, pero la limpieza del efecto que cancela el temporizador es PASIVA y corre después del commit; si el temporizador de 450 ms dispara en el medio -y acá dispara justo, porque la Server Action revalida la ruta y termina alrededor de esa marca-, `setMontado(true)` entra con `visible` ya en `false` y desde ahí `visible === visiblePrevio` para siempre. Arreglado con una condición derivada (`montado && visible`) y **dos tests de regresión** que fallan contra el código viejo (verificado revirtiendo el arreglo: 2 failed | 9 passed).

**Verificación del circuito OAuth contra el dev server** (30/30 PASS, script ad-hoc con sesión real, sin imprimir ningún secreto): `/perfil/gmail` sin sesión → login; `/api/gmail/conectar` sin sesión → redirección amable al login (no 401 JSON); con sesión → **302 a `accounts.google.com/o/oauth2/v2/auth`** con el `client_id` del entorno (comparado contra `process.env`, no impreso), `redirect_uri=http://localhost:3000/api/gmail/callback` exacto, los TRES scopes, `access_type=offline` + `prompt=consent`, y un `state` opaco de 32 bytes que **no lleva el user_id**; la cookie del sobre firmado con `HttpOnly`, `SameSite=Lax`, `Path=/api/gmail`, `Max-Age=600` y sin `Secure` en `http://localhost`. Y los cuatro rechazos del callback: `error=access_denied` → "cancelada"; `state` falso sin cookie → "estado_invalido"; cookie válida con `state` que no matchea → "estado_invalido"; **firma del sobre alterada** → "estado_invalido"; sin sesión → 303 al login.

**Suites del cierre:** `npx tsc --noEmit` limpio · `npx eslint .` limpio · `npm run test` **1011/1011** · `node scripts/verificar-contraste.mjs` **196/196** · RLS `scripts/test-rls.sql` **420/420 ×2** (382 previos + 38 del BLOQUE 23) · `npm run build` limpio con el puerto 3000 verificado por `netstat` antes.

**Data de desarrollo:** ninguna. La conexión de prueba (`casilla.de.prueba@gmail.com`, token de mentira) se creó y se borró dentro de esta verificación; la base local terminó con **0 filas en `gmail_connections` y 0 secretos `gmail_refresh_token_*` en el Vault**, confirmado por SQL.

## Sprint 16 · tarea 16.4 — Pegá el mensaje de la clínica: la IA precarga el turno

**2026-08-18.** Samsung Galaxy A71 real (`R58N85AW49F`), `adb reverse tcp:3000 tcp:3000` + `adb forward tcp:9222 localabstract:chrome_devtools_remote`, CDP nativo por WebSocket de Node 24 (script ad-hoc, mismo método que el resto de este archivo). Login real de María (`maria@ejemplo.com.ar` / `password123`) por CDP (setter nativo de `HTMLInputElement` + `dispatchEvent`), selección del perfil gestionado de Roberto en `/perfiles` (toque real), y navegación directa a `/turnos/nuevo`.

**Flujo real de punta a punta:**

1. `/turnos/nuevo` mostró la sección colapsada nueva "¿Te llegó el turno por WhatsApp? Pegalo acá", arriba del formulario de siempre, sin mover ni un campo existente.
2. Un TOQUE REAL sobre el botón la expandió: textarea "Mensaje de la clínica", el aviso de minimización ("Se lo mandamos a una inteligencia artificial (Gemini) solo para leerlo — no se guarda. Si trae nombre o DNI del paciente, se ignoran") y el botón "Analizar".
3. Se pegó el mensaje REAL del fixture `tests/fixtures/mensajes-turno/clinica-san-jorge-ecografia.txt` -texto completo, con el DNI y el nombre del paciente incluidos, tal como llegaría por WhatsApp- con el mismo método de "paste" que usa el resto de este archivo para tipeo (setter nativo de `HTMLTextAreaElement` + `dispatchEvent`, equivalente a lo que hace el sistema operativo al pegar del portapapeles). Capturado en `sprint16-analizar-pegado.png`.
4. TOQUE REAL sobre "Analizar": la llamada disparó `POST /api/turnos/analizar-mensaje` de verdad (sin mocks), que llamó a Gemini real. `VeloEspera` apareció con "La inteligencia artificial está leyendo el mensaje…" mientras la llamada estaba en vuelo -confirmado en el DOM en el momento de la foto-.
5. ~6 segundos después (round-trip real contra la API de Gemini, no local), el formulario quedó PRECARGADO. Leído directo del DOM (`fecha.value`, `hora.value`, etc., no una inferencia visual):
   ```json
   { "fecha": "2024-10-07", "hora": "14:15", "especialidad": "Ecografía",
     "medico": "Dr. Juárez", "lugarNombre": "ANEXO DR JORGE SAGARDIA",
     "lugarDireccion": "DE LA ESTANCIA 1955", "notasLargo": 749 }
   ```
   Coincide EXACTO con la receta de verificación del encargo: fecha 7 de octubre, 14:15, ecografista, Dr. Juárez normalizado (de "SERV. DE ECOGRAFIA - DR. JUAREZ" a "Dr. Juárez"), dirección De la Estancia 1955, preparación en notas (749 caracteres: coseguro + preparación + recordatorio de asistir con orden médica). Capturado en `sprint16-analizar-precargado.png`.
6. **El DNI y el nombre del paciente del mensaje** ("DN12345678 PEREZ JUAN CARLOS ALBERTO", visibles en la captura del paso 3 porque son parte del texto pegado) **no aparecen en ningún campo del formulario precargado** -verificado leyendo los nueve campos precargables del DOM y buscando las dos cadenas, cero apariciones-, confirmando en el dispositivo real el criterio de minimización de `docs/minimizacion-datos.md` §9.
7. Ciudad/Provincia quedaron en "Ushuaia"/"Tierra del Fuego, Antártida e Islas del Atlántico Sur" -la sugerencia de última ubicación usada (tarea 16.1) que ya traía el formulario antes de analizar nada-: el mensaje de San Jorge no menciona ciudad ni provincia, y `aplicarPrecarga` (`lib/turnos/aplicar-precarga.ts`) nunca vacía un campo cuando la propuesta de la IA no trae nada para él.

**No se guardó ningún turno real**: la verificación se detuvo en la revisión del formulario precargado, sin tocar "Guardar turno" -el criterio de aceptación de la tarea es la precarga, no el alta-, así que no queda ningún turno de prueba en la base local.

**Suites completas corridas para esta tarea:** `npx tsc --noEmit` limpio; `npx eslint .` limpio; `npm run test` → **966/966** (903 preexistentes + 63 nuevos: `normalizacion-mensaje-turno.test.ts`, `mapear-especialidad-catalogo.test.ts`, `aplicar-precarga-turno.test.ts`, `construir-propuestas-turno.test.ts`, `analisis-turno-schema.test.ts`); `node scripts/verificar-contraste.mjs` → **196/196 PASS, 0 fallas** (sin tokens de color nuevos: la sección nueva reutiliza `Alerta` y `Boton` ya verificados); `npm run build` → build de producción exitoso, con el dev server detenido y confirmado con `netstat` antes y reiniciado después. RLS: N/A -esta tarea no tocó ninguna migración, entra entera en los campos existentes de `appointments`-. Análisis real contra Gemini de los 8 fixtures + los 2 pares (dividir/fusionar) + corrida doble de estabilidad: `scripts/test-analizar-mensaje.mjs`, tabla completa en el Resumen de Entrega de la tarea.
...
| sprint16-direcciones-formulario.png | `/turnos/nuevo` (tarea 16.1) en el dispositivo real, sesión de María sobre el perfil gestionado de Roberto: los campos NUEVOS "Ciudad"/"Provincia" con datos reales tipeados — "Avenida 51 Nº 315" en Dirección, "La Plata" en Ciudad, "Buenos Aires" elegida del `<Select>` de las 24 jurisdicciones (reemplazando el default sugerido "Tierra del Fuego..." que había precargado `lib/ubicacion/ultima-usada.ts` a partir del último turno cargado, Ushuaia) — dirección real del fixture `tests/fixtures/mensajes-turno/instituto-medico-platense-traumatologia.txt` |
| sprint16-direcciones-llegar.png | `/turnos` justo después de guardar ese turno de verdad: la tarjeta "Traumatología" muestra "Instituto Médico Platense — Avenida 51 Nº 315, La Plata, Buenos Aires" y su botón "Llegar" -confirmado por lectura del `href` real, no una inferencia visual- apunta a `https://www.google.com/maps/dir/?api=1&destination=-34.918656%2C-57.952597` -coordenadas REALES de La Plata, resueltas por Nominatim en el momento de guardar, cero mención a Ushuaia-, mientras la tarjeta de al lado ("Endocrinología", turno del seed sin tocar) sigue mostrando correctamente "Maipú 345, Ushuaia, Tierra del Fuego..." -confirma que cada turno muestra SU propia ciudad real, no una asumida para todos- |
| sprint16-especialidades-autocompletar.png | `/medicos/nuevo` (tarea 16.2) en el dispositivo real, sesión de María sobre Roberto: campo "Especialidades" con DOS chips ya cargados ("Cardiología", "Clínica Médica") tras dos rondas reales de tipear+tocar la sugerencia -"cardio" (sin tilde) hizo aparecer "Cardiología" Y "Cirugía Cardiovascular" a la vez, confirmando el matching por subcadena insensible a tildes/mayúsculas, no solo por prefijo-, con el campo de borrador vacío y listo para la próxima, y "Dra. Julia Fernández" ya tipeada en "Nombre del médico" |
| sprint16-especialidades-tarjeta.png | `/medicos` justo después de guardar esa alta de verdad: la tarjeta de "Dra. Julia Fernández" muestra "Cardiología · Clínica Médica" -las DOS especialidades cargadas por autocompletar, unidas con `·`, en el mismo orden en que se agregaron-, junto a la de "Dr. Carlos Rodríguez" (del seed, ahora con "Clínica Médica · Cardiología") y la de "Dra. Marcela Torres" (una sola especialidad, sin cambios de layout) |
| sprint16-lugares-sync.png | Velo de espera de la sincronización del catálogo REFES (tarea 16.3) en el dispositivo real, a los 5,9 s de un TAP REAL en "Actualizar": **"Sincronizando 17.331 de 36.046 centros"** con el número exacto -el total se cuenta durante la preparación, no se estima- y el submensaje "No cierres esta pantalla. Si se corta, la próxima vez sigue desde acá". Detrás corre la descarga real del CSV del Ministerio de Salud y las 9 tandas de upsert |
| sprint16-lugares-busqueda.png | `/lugares` tras una búsqueda REAL escrita en el teléfono ("san jorge" en Buscar + "ushuaia" en Localidad, botón "Buscar" tocado de verdad): **7 centros encontrados**, todos de Ushuaia, con el tipo de establecimiento del registro ("Otros (ESSIT)", "Con atención médica diaria..."), el domicilio completo con la provincia CANÓNICA ("Tierra del Fuego, Antártida e Islas del Atlántico Sur", no el "TIERRA DEL FUEGO" del REFES), el CP y los botones "Llegar"/"Viaje". Los `href` de "Llegar" se leyeron del DOM: `...destination=-54.815585%2C-68.324898`, coordenadas REALES del registro oficial — cero geocodificación |
| sprint16-lugares-al-dia.png | El mismo `/lugares` tras tocar "Actualizar" con el catálogo YA sincronizado: **"Ya tenés la edición más nueva que publicó el Ministerio: 36.046 centros."** en 311 ms, sin descargar los 9 MB -la comparación es un `HEAD` contra el archivo del portal, ver el Resumen de Entrega- y con el enlace "Descargarla de nuevo igual" por si el catálogo local quedó a medias |
| sprint16-inicio-grilla.png | Grilla de accesos de `/inicio` con las DOS cards nuevas de la tarea 16.3, **Especialidades** y **Lugares**: 8 tiles en 3 filas (3+3+2) sin tocar ni una clase de la grilla del Sprint 14. Los 8 títulos entran sin truncar, medido por CDP (`scrollWidth === clientWidth` en los 8; "Especialidades" mide 98px de los ~105px disponibles) |
| sprint16-turno-autocompletar.png | `/turnos/nuevo` con el campo "Lugar" convertido en buscador del catálogo (tarea 16.3): tras tipear "clinica san jorge" con `Input.insertText` (el mismo camino que el teclado del sistema) el desplegable muestra las coincidencias del REFES con su dirección debajo de cada nombre. El popup se abre HACIA ARRIBA porque no entraba abajo, sin taparse con la bottom nav |
| sprint16-turno-precargado.png | El mismo formulario justo después de un TOQUE REAL sobre la primera sugerencia: "Lugar" quedó en "ANEXO DR. R LIVINGSTON CLINICA SAN JORGE" y se completaron solas **Dirección** ("JAINEN N° 136"), **Ciudad** ("USHUAIA"), **Provincia** ("Tierra del Fuego, Antár…") y el panel de coordenadas, que se abrió solo con **-54.815585 / -68.324898** del registro oficial. Es el broche de la tarea: cero geocodificación y cero números copiados a mano |
| sprint16-especialidades-dispositivo.png | `/especialidades` (tarea 16.3) en el dispositivo real: los médicos de Roberto agrupados por especialidad, con **Dr. Carlos Rodríguez apareciendo bajo Cardiología Y bajo Clínica Médica** -sus dos especialidades de la tarea 16.2- y Dra. Marcela Torres bajo Endocrinología. Jerarquía de encabezados verificada por CDP: `H1 Especialidades → H2 Cardiología → H3 Dr. Carlos Rodríguez` |
| sprint16-analizar-pegado.png | `/turnos/nuevo` (tarea 16.4) en el dispositivo real: la sección "¿Te llegó el turno por WhatsApp? Pegalo acá" abierta con el mensaje REAL del fixture de la Clínica San Jorge pegado en el textarea -DNI y nombre de paciente incluidos, tal como llega el mensaje real, con el aviso de minimización visible arriba ("Se lo mandamos a una inteligencia artificial... no se guarda... se ignoran")- y el botón "Analizar" listo para tocar |
| sprint16-analizar-precargado.png | El mismo formulario, con scroll hasta los campos, justo después de la llamada REAL a Gemini: **Especialidad "Ecografía"**, **Médico "Dr. Juárez"** (normalizado desde "SERV. DE ECOGRAFIA - DR. JUAREZ" del mensaje), **Fecha "07/10/2024"**, **Hora "2:15 p.m."** (14:15), **Lugar "ANEXO DR JORGE SAGARDIA"** y **Dirección "DE LA ESTANCIA 1955"** — los seis campos coinciden exactamente con lo que pedía la receta de verificación de la tarea. Ciudad/Provincia quedaron en "Ushuaia"/"Tierra del Fuego..." -la sugerencia de última ubicación usada (tarea 16.1), intacta porque el mensaje no las menciona y la precarga de IA nunca vacía un campo que no tiene con qué llenar-. El DNI y el nombre del paciente del mensaje NO aparecen en ningún campo |

## Sprint 16 · tarea 16.2 — Especialidades: catálogo, autocompletar y multi-especialidad

**2026-08-18.** Samsung Galaxy A71 real (`R58N85AW49F`), `adb reverse tcp:3000 tcp:3000` + `adb forward tcp:9222 localabstract:chrome_devtools_remote`, CDP nativo por WebSocket de Node 24 (mismo cliente sin librerías del resto de este archivo). Login real de María (`maria@ejemplo.com.ar` / `password123`, el mismo seed de siempre) por CDP -setter nativo de `HTMLInputElement` + `dispatchEvent`, igual que la 16.1-, selección real del perfil gestionado de Roberto.

**Flujo real de punta a punta, alta de un médico nuevo con DOS especialidades, con toques reales:**

1. `/medicos/nuevo`: se tipeó "Dra. Julia Fernández" en "Nombre del médico" (setter nativo + `dispatchEvent`).
2. Se tocó el campo "Especialidades" (`Input.dispatchMouseEvent`, tap real) y se tipeó `cardio` -sin tilde, minúscula-: el desplegable mostró **dos** coincidencias, "Cardiología" **y** "Cirugía Cardiovascular" -la segunda matchea por SUBCADENA en el medio de la palabra ("cirugía **cardio**vascular"), no por prefijo, confirmando que `especialidadCoincide` (`lib/especialidades/coincidencias.ts`) hace `contains`, no `startsWith`-.
3. Se tocó "Cardiología" con un tap real sobre sus coordenadas reales (`getBoundingClientRect()` del ítem, no una posición supuesta): el chip apareció DE UNA, sin pasar por "Agregar" -confirma el criterio `reason === "item-press"` de `CampoEspecialidades`-.
4. Se tocó de nuevo el campo (ya vacío) y se tipeó `clinica` -sin tilde-: el desplegable mostró "Clínica Médica" (matchea con tilde en el catálogo real). Se tocó con un tap real: segundo chip agregado (`sprint16-especialidades-autocompletar.png`, los dos chips visibles: "Cardiología" ×, "Clínica Médica" ×).
5. Se tocó "Guardar médico" (tap real sobre las coordenadas reales del botón, tras `scrollIntoView`): la Server Action `crearMedico` corrió contra Supabase LOCAL de verdad y redirigió a `/medicos?creado=1`.
6. `/medicos`: la tarjeta nueva de "Dra. Julia Fernández" muestra **"Cardiología · Clínica Médica"** -las dos, unidas, en el orden en que se cargaron- junto a la de "Dr. Carlos Rodríguez" (sembrada en el seed de esta misma tarea con `specialties = {"Clínica Médica","Cardiología"}`, mostrando **"Clínica Médica · Cardiología"**) (`sprint16-especialidades-tarjeta.png`).
7. `/turnos/nuevo`: se abrió el `<Select>` "Médico (opcional)" -sus opciones ya muestran las especialidades unidas: "Dra. Julia Fernández — Cardiología · Clínica Médica"- y se tocó (tap real sobre las coordenadas reales del ítem) "Dra. Julia Fernández": el campo "Especialidad" se precargó con **"Cardiología"** -la PRINCIPAL, la primera que se cargó, no las dos ni la última- y "Médico" con "Dra. Julia Fernández", confirmado leyendo `document.getElementById('especialidad').value` en el DOM real, no por inspección visual.

**Verificación cruzada contra la base LOCAL** (no solo la pantalla), corrida después del alta real:

```sql
select full_name, specialties from public.doctors
 where profile_id = '660e8400-e29b-41d4-a716-446655440003'::uuid
 order by full_name;
-- Dr. Carlos Rodríguez  | {"Clínica Médica",Cardiología}
-- Dra. Julia Fernández  | {Cardiología,"Clínica Médica"}
-- Dra. Marcela Torres   | {Endocrinología}
```

Los tres médicos coinciden exactamente con lo que mostró la pantalla: el orden del array se conserva tal cual se cargó, sin ordenar alfabéticamente ni reordenar.

**Un hallazgo de metodología, no del producto (declarado para que quede claro que no es un bug):** al re-enfocar el campo de borrador programáticamente (`el.focus()` vía CDP, SIN un tap real de por medio) y tipear con el mismo truco del setter nativo, el desplegable de sugerencias NO se reabrió solo -`@base-ui/react` gatea el auto-apertura por tipeo detrás de haber visto un `keydown` real (`keyboardActiveRef`), y un `dispatchEvent(new Event('input'))` sintético no dispara `keydown`-. Con un tap real sobre el campo (el mismo `Input.dispatchMouseEvent` que ya se usaba para todo lo demás) el desplegable volvió a abrirse y a filtrar correctamente. Un teclado nativo de Android SÍ genera `keydown` reales en cada tecla, así que esto no afecta a ninguna persona usando la app; es una particularidad de simular tipeo por JavaScript sin un evento de teclado real de por medio, que solo importa para quien automatiza pruebas contra este componente en particular -documentado acá para que una sesión futura no lo redescubra ni lo confunda con un bug-.

**Suites completas corridas para esta tarea:** `npx tsc --noEmit` limpio; `npx eslint .` limpio; `npm run test` → **828/828** (810 previos + 18 nuevos: 10 de `coincidencias-especialidad.test.ts` -tildes, mayúsculas, prefijos y subcadenas-, 7 sumados a `medico-schema.test.ts` -trim, dedupe, tope de cantidad y de largo-, 1 sumado a `autocompletar-medico.test.ts` -la especialidad PRINCIPAL cuando el médico tiene varias-); `node scripts/verificar-contraste.mjs` → **196/196 PASS** (sin tokens de color nuevos: `components/ui/autocomplete.tsx` reutiliza `border-input`, `bg-popover`, `text-popover-foreground`, `bg-accent`/`text-accent-foreground`, `text-destructive`, todos ya verificados por `Select`/`Input`); `npm run build` exitoso con el dev server detenido (`netstat -ano | grep :3000` confirmado antes) y reiniciado al terminar. RLS: `scripts/test-rls.sql` → **357/357 PASS × 3 corridas consecutivas**, sin casos nuevos -la migración reemplaza `doctors.specialty` por `doctors.specialties` (columna por columna, misma fila, mismo `profile_id`) y `doctors` no tiene ningún bloque dedicado en el arnés: queda cubierta por el chequeo genérico de `rowsecurity`/`pg_policies` sobre todas las tablas de `public`, que no depende de qué columnas tenga cada una-.

## Sprint 16 · tarea 16.1 — Ciudad y provincia: el fin del sesgo a Ushuaia

**2026-08-18.** Samsung Galaxy A71 real (`R58N85AW49F`), `adb reverse tcp:3000 tcp:3000` + `adb forward tcp:9222 localabstract:chrome_devtools_remote`, CDP nativo por WebSocket de Node (mismo método que el resto de este archivo). Login real de María (`maria@ejemplo.com.ar`) por CDP (setter nativo de `HTMLInputElement` + `dispatchEvent`), perfil de Roberto Gómez.

**El hallazgo:** el bug reportado por el usuario ("las direcciones de turnos y médicos quedan forzadas a Ushuaia") NO vivía en ninguna llamada a Nominatim -no existía ninguna antes de esta tarea, pese a que el roadmap la mencionaba como "patrón del proyecto"-. Vivía en `lib/logistica/deep-links.ts#linkComoLlegar`: cuando un turno no tenía coordenadas, la función pegaba ", Ushuaia, Tierra del Fuego" a CUALQUIER dirección antes de armar el link de Google Maps. Un turno cargado en La Plata terminaba con un "Cómo llegar" que buscaba "Avenida 51 Nº 315, Ushuaia, Tierra del Fuego" -o bien resolvía al lugar equivocado, o Google Maps no encontraba nada-.

**Verificación de punta a punta con datos reales, tocando la pantalla:**

1. `/turnos/nuevo`: se tipeó la dirección real del fixture del Sprint 16 (`Avenida 51 Nº 315`, `La Plata`, `Buenos Aires` — el mensaje real de WhatsApp del Instituto Médico Platense) sin cargar coordenadas a mano (`sprint16-direcciones-formulario.png`).
2. Al guardar, la Server Action `crearTurno` geocodificó AUTOMÁTICAMENTE contra Nominatim (`lib/ubicacion/geocodificacion.ts`, request real a `nominatim.openstreetmap.org`, no simulado) y guardó `latitude = -34.918656, longitude = -57.952597` -verificado contra la base LOCAL, no solo la pantalla-:
   ```sql
   select specialty, location_address, location_city, location_province, latitude, longitude
     from public.appointments where specialty = 'Traumatología';
   -- Traumatología | Avenida 51 Nº 315 | La Plata | Buenos Aires | -34.918656 | -57.952597
   ```
   Las coordenadas caen dentro de La Plata real (el centro de la ciudad está en, aproximadamente, -34.92/-57.95) y a **más de 2.100 km** de Ushuaia (-54.8/-68.3) — no es un redondeo ni una coincidencia, es la ciudad correcta.
3. `/turnos`: el botón "Llegar" de esa tarjeta se leyó por `href` real (`sprint16-direcciones-llegar.png`) y apunta a `destination=-34.918656,-57.952597` — las coordenadas de La Plata, cero Ushuaia.
4. **Regresión de turnos viejos, en la misma pantalla:** la tarjeta de al lado ("Endocrinología", turno del seed que nunca se tocó en esta tarea) sigue mostrando su dirección real de Ushuaia sin ningún cambio de comportamiento — cada turno muestra SU ciudad, no una fija para todos.
5. `/medicos/nuevo`: el `<Select>` de provincia llegó precargado con "Buenos Aires" -la última ubicación que cargó este perfil, ahora el turno de Traumatología recién guardado-, confirmando que `lib/ubicacion/ultima-usada.ts` sigue la actividad real del perfil y no un valor fijo.
6. `/turnos/[id]/editar` del turno de Traumatología: los campos "Ciudad"/"Provincia" llegan precargados con "La Plata"/"Buenos Aires" tal como se guardaron.

**Suites completas corridas para esta tarea:** `npx tsc --noEmit` limpio; `npx eslint .` limpio; `npm run test` → **810/810** (774 previos + 36 nuevos: 7 de `provincias.test.ts`, 8 de `direccion-completa.test.ts`, 8 de `geocodificacion.test.ts` con `fetch` mockeado, más casos sumados a `deep-links.test.ts`, `autocompletar-medico.test.ts`, `turno-schema.test.ts` y `medico-schema.test.ts`); `node scripts/verificar-contraste.mjs` → **196/196 PASS** (sin tokens de color nuevos: los campos Ciudad/Provincia reusan `CampoTexto`/`Select` ya verificados); `npm run build` exitoso con el dev server detenido (`netstat` confirmado antes y después) y reiniciado al terminar. RLS: `scripts/test-rls.sql` → **357/357 PASS × 2 corridas consecutivas**, sin casos nuevos -la migración solo agrega columnas nullable a tablas ya cubiertas por `appointments_update_administrador`/`doctors_update_administrador`, que operan por fila y no por columna-.
...
## Sprint 16 · tarea 16.3 — Catálogo REFES: Actualizar, Lugares y Especialidades

**2026-08-18.** Samsung Galaxy A71 real (`R58N85AW49F`), `adb reverse tcp:3000 tcp:3000` + `adb forward tcp:9222 localabstract:chrome_devtools_remote`, CDP nativo por WebSocket de Node 24. Login real de María por CDP y selección real del perfil gestionado de Roberto, como en las tareas anteriores del sprint.

**La sincronización se corrió DE VERDAD contra el portal del Ministerio de Salud** (`datos.salud.gob.ar`), no contra un fixture: el catálogo se vació antes de cada corrida y se llenó con las 36.046 filas del CSV oficial de diciembre de 2025.

**Flujo completo, con toques reales:**

1. `/lugares` con el catálogo VACÍO muestra el estado inicial ("Todavía no descargaste el catálogo", con la explicación de qué va a pasar al tocar Actualizar).
2. **TAP REAL en "Actualizar"** (`Input.dispatchMouseEvent` sobre las coordenadas CSS del botón). El velo de espera aparece de inmediato con "Buscando la edición más nueva del Ministerio" y a los ~4 s pasa a los números reales (`sprint16-lugares-sync.png`). Progreso leído del DOM en cada muestreo:

   | t | mensaje del velo |
   |---|---|
   | 0,8–3,7 s | Buscando la edición más nueva del Ministerio |
   | 4,4 s | Sincronizando 0 de 36.046 centros |
   | 5,5 s | Sincronizando 8.527 de 36.046 centros |
   | 5,9 s | Sincronizando 17.331 de 36.046 centros ← la captura |
   | 8,3 s | Sincronizando 25.922 de 36.046 centros |
   | 9,8 s | (velo cerrado) |

3. Al terminar, en pantalla: **"Catálogo actualizado: 36.046 centros de salud."** y el encabezado pasa a "36.046 centros · última actualización: 18 de agosto de 2026".
4. **Segundo toque en "Actualizar"**, ahora con el catálogo al día: responde en **311 ms** con "Ya tenés la edición más nueva que publicó el Ministerio: 36.046 centros." (`sprint16-lugares-al-dia.png`) — sin descargar los 9 MB.
5. **Búsqueda real** tipeada en el teléfono: "san jorge" + localidad "ushuaia" → 7 centros, todos de Ushuaia (`sprint16-lugares-busqueda.png`). Los `href` de "Llegar" se leyeron del DOM y llevan las coordenadas del registro (`destination=-54.815585%2C-68.324898`), no una búsqueda por texto.
6. **`/turnos/nuevo`**: el campo "Lugar" busca en el catálogo mientras se tipea (`sprint16-turno-autocompletar.png`) y un **toque real** sobre una sugerencia completa dirección, ciudad, provincia y coordenadas (`sprint16-turno-precargado.png`).
7. **`/especialidades`**: los médicos de Roberto agrupados, con el que tiene dos especialidades apareciendo en las dos (`sprint16-especialidades-dispositivo.png`).
